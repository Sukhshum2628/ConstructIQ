...
class ProjectModel {
  ...
  factory ProjectModel.fromJson(Map<String, dynamic> json) {
    return ProjectModel(
      ...
      expectedEndDate: (json['expectedEndDate'] as Timestamp?)?.toDate() ?? DateTime.now().add(const Duration(days: 90)),
      ...
      plannedBudget: (json['plannedBudget'] as num? ?? 0.0).toDouble(),
      projectType: json['projectType'] as String? ?? 'other',
      cadFileUrl: json['cadFileUrl'] as String? ?? '',
      estimationStatus: EstimationStatus.values.firstWhere(
        (e) => e.name == json['estimationStatus'],
        orElse: () => EstimationStatus.pending,
      ),
